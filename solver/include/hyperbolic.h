...
public:
    HyperbolicSolver(std::shared_ptr<Differentiator<T>> differentiator)
        : differentiator_(differentiator), integrator_(std::make_shared<RungeKutta4<T>>()) {}
    ...
    void initialize(int N, T t_final) {
        ...
        differentiator_->compute_analy_sols(TestFunctions::hyperbolic_initial_u<T>, TestFunctions::hyperbolic_du_t0<T>);
    ...
    std::tuple<std::vector<T>, std::vector<T>, std::vector<T>, T, double> get_results() const {
        return {differentiator_->get_x(), u_, u_exact_, compute_error(), computation_time_};
    }
...
void convergence_study() {
    ...
    std::cout << std::setw(8) << "N" << std::setw(20) << "Second Order" 
              << std::setw(20) << "Fourth Order" << std::setw(20) << "Fourier" << std::endl;
    ...
    for (size_t i = 0; i < N_values.size(); ++i) {
        ...
        std::cout << std::scientific << std::setprecision(6) << std::setw(20) << error_second;
        ...
        solver_fourth.solve();
        auto [____, _____, ______, error_fourth, time_fourth] = solver_fourth.get_results();
        errors_fourth[i] = error_fourth;
        times_fourth[i] = time_fourth;
        std::cout << std::setw(20) << error_fourth;
        
        // Fourier method
        auto fourier = std::make_shared<SpectralFourier<T>>(MethodType::EVEN);
        ...
        auto [_______, ________, _________, error_fourier, time_fourier] = solver_fourier.get_results();
    ...
    for (size_t i = 1; i < N_values.size(); ++i) {
        ...
        std::cout << std::setw(8) << N_values[i]
                 << std::fixed << std::setprecision(2) << std::setw(20) << rate_second
                 << std::setw(20) << rate_fourth
                 << std::setw(20) << rate_fourier << std::endl;
    }
    
    // Compare N values needed for same accuracy
    T error_second_2048 = errors_second.back();
    std::cout << "\nError for N=2048 using second order: " << std::scientific << error_second_2048 << std::endl;
    ...
    for (size_t i = 0; i < errors_fourth.size(); ++i) {
        if (errors_fourth[i] <= error_second_2048) {
            ...
            if (i > 0) {
                ...
                N_eq_fourth = N_values[i-1] + (N_values[i] - N_values[i-1]) * 
    ...
    for (size_t i = 0; i < errors_fourier.size(); ++i) {
        if (errors_fourier[i] <= error_second_2048) {
            ...
            if (i > 0) {
                // Interpolate between values
                N_eq_fourier = N_values[i-1] + (N_values[i] - N_values[i-1]) * 
...
void long_time_integration() {
    ...
    for (T t_final : times) {
        ...
        solver_fourier.solve();
        auto [____, _____, ______, error_fourier, time_fourier] = solver_fourier.get_results();
