...
public:
  ...
  virtual std::vector<T> compute_F(const std::vector<T> &u) const; // Added const overload
...
std::vector<T> Differentiator<T>::compute_F(const std::vector<T> &u) const {
  // Create a copy of the input since we can't modify analytical_u_ in a const method
  std::vector<T> u_copy = u;
  std::vector<T> du(u.size());
  
  // Manually compute the derivative based on the derivative type
  // This is a simplified version that might need to be specialized for different derivative types
