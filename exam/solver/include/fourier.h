...
public:
  ...
  //   return u_hat;
  // }
  // CORRECTED: Compute Fourier coefficients from physical space values
  std::vector<std::complex<T>> fft(const std::vector<T> &u) {
    int N = u.size() - 1;
    std::vector<std::complex<T>> u_hat(N + 1);
    ...
    for (int k = -N / 2; k <= N / 2; ++k) {
      ...
      for (int j = 0; j <= N; ++j) {
        T arg = -T(2) * MathConstants<T>::PI() * T(k) * T(j) / T(N + 1);
        sum += u[j] * std::complex<T>(cos(arg), sin(arg));
      }
      // CORRECTED indexing: map k ∈ [-N/2, N/2] to idx ∈ [0, N]
      int idx = (k >= 0) ? k : (N + 1 + k);
      u_hat[idx] = sum / T(N + 1);
    }
    return u_hat;
  ...
  std::vector<T> ifft(const std::vector<std::complex<T>> &u_hat) {
    ...
    for (int j = 0; j <= N; ++j) {
      ...
      for (int k = -N / 2; k <= N / 2; ++k) {
        ...
        int idx = (k >= 0) ? k : (N + 1 + k);
        T arg = T(2) * MathConstants<T>::PI() * T(k) * T(j) / T(N + 1);
        sum += u_hat[idx] * std::complex<T>(cos(arg), sin(arg));
      }
      u[j] = sum.real();
    }
    return u;
  ...
  compute_burgers_spectral_rhs(const std::vector<T> &u, T nu) {
    ...
    int N = nonlinear_hat.size() - 1;
    int max_mode = N / 2;
    int dealias_cutoff = (2 * max_mode) / 3; // 2/3 rule

    for (int k = -max_mode; k <= max_mode; ++k) {
      int idx = (k >= 0) ? k : (N + 1 + k);

      // Zero out high-frequency modes
      if (abs(k) > dealias_cutoff) {
        nonlinear_hat[idx] = std::complex<T>(0, 0);
      }
    }

    // Compute full RHS in spectral space: -nonlinear + diffusion
    std::vector<std::complex<T>> rhs_hat(u_hat.size());
    for (size_t i = 0; i < u_hat.size(); ++i) {
      rhs_hat[i] = -nonlinear_hat[i] + nu * d2u_hat[i];
    }
